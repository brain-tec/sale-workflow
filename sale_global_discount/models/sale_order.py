...
class SaleOrderLine(models.Model):
    ...
    def _convert_to_tax_base_line_dict(self):
        # OVERRIDE
        self.ensure_one()
        if not self.order_id.global_discount_ids or not self.env.context.get(
            "from_tax_calculation", False
        ):
            return super()._convert_to_tax_base_line_dict()
        discounts = self.order_id.global_discount_ids.mapped("discount")
        discounted_price_unit = self.order_id.get_discounted_global(
            self.price_unit, discounts.copy()
        )
